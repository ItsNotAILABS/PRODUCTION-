...
const ALPHA_DEPLOYMENTS = {
  ...
  'marketplace-sdk': {
    ...
    type: 'npm-package',
    workflow: 'deploy-marketplace-sdk.yml',
...
function generateReport() {
  ...
  lines.push('## 🔷 Alpha Deployments');
  lines.push('');
  lines.push('Production-ready alpha deployments managed by the fleet:');
  lines.push('');
  lines.push('| Deployment | Type | Workflow | Status |');
  lines.push('|------------|------|----------|--------|');

  for (const [id, deployment] of Object.entries(ALPHA_DEPLOYMENTS)) {
    const workflowPath = path.join(WORKFLOWS, deployment.workflow);
    const hasWorkflow = fs.existsSync(workflowPath);
    const statusIcon = hasWorkflow ? '✅ Ready' : '⚠️ Pending';
    lines.push(`| ${deployment.emoji} ${deployment.name} | ${deployment.type} | \`${deployment.workflow}\` | ${statusIcon} |`);
  }

  lines.push('');
  lines.push('### Twin Alpha Runtime Interface');
  lines.push('');
  lines.push('The **Geminus Primus** — always-on user interface to the organism.');
  lines.push('');
  lines.push('- **URL**: ' + (ALPHA_DEPLOYMENTS['twin-alpha'].url || 'N/A'));
  lines.push('- **Type**: Static Web App with Service Worker');
  lines.push('- **Workers**: 22+ (neuro-core, twin-alpha, memory, engine, inference, routing, etc.)');
  lines.push('');
  lines.push('### Marketplace SDK');
  lines.push('');
  lines.push('The **callable tool marketplace** — 24 AI-invocable tools.');
  lines.push('');
  lines.push('- **Package**: ' + (ALPHA_DEPLOYMENTS['marketplace-sdk'].package || 'N/A'));
  lines.push('- **Families**: Crawling (6), Context (6), Commander (6), Sentry (6)');
  lines.push('- **Adapters**: ChatGPT, OpenAI, Claude');
  lines.push('');
