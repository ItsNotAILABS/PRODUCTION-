...
function verifyWiring() {
  ...
  const warned = wiring.filter(w => w.status === 'warn').length;
  phaseResult('Cross-Module Wiring', '🔌', warned === 0 ? 'pass' : 'warn', {
    connections: wiring, warned, duration: Date.now() - phaseStart,
  });
}
...
function generateReport() {
  ...
  const hasWarnings = phases.some(p => p.status === 'warn');
  ...
  for (const p of phases) {
    ...
    else if (p.connections) detail = `${p.connections.length} connections, ${p.warned || 0} warned`;
  ...
  const summary = {
    ...
    phases: phases.map(p => ({
      ...
      detail: p.valid !== undefined ? `${p.valid} valid, ${p.invalid} invalid`
        ...
        : p.connections ? `${p.connections.length} connections, ${p.warned || 0} warned`
        : p.message || '',
    })),
  };
